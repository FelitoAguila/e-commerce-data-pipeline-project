...
def plot_real_vs_predicted_delivered_time(df: pd.DataFrame, year: int):
...
def plot_global_amount_order_status(df: pd.DataFrame):
...
def plot_revenue_per_state(df: pd.DataFrame):
...
def plot_top_10_least_revenue_categories(df: pd.DataFrame):
    ...
    ax.legend(
        wedges,
        elements,
        title="Least Revenue Categories",
        loc="center left",
        bbox_to_anchor=(1, 0, 0.5, 1),
    )
...
def plot_top_10_revenue_categories_ammount(df: pd.DataFrame):
...
def plot_delivery_date_difference(df: pd.DataFrame):
...
def plot_order_amount_per_day_with_holidays(df: pd.DataFrame):
    ...
    plt.title("Orders per day during the 2017")
    plt.xlabel("Date")
    plt.ylabel("Number of orders")


    for holiday_date in df[df['holiday']]['date']:
        plt.axvline(x=holiday_date, 
                    color='purple', 
                    ls='--', 
                    linewidth=0.8)

    plt.legend(['Orders', 'Holiday dates'])
    plt.show()
