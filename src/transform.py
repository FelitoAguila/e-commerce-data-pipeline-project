...
def query_delivery_date_difference(database: Engine) -> QueryResult:
    ...
    return QueryResult(query=query_name, result=pd.read_sql(query, database))
...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    items = pd.read_sql("SELECT * FROM olist_order_items", database)

    # Get products from olist_products table
    products = pd.read_sql("SELECT * FROM olist_products", database)

    # Merge items, orders and products tables on 'order_id'/'product_id'.
    # Assign the result to the `data` variable.
    data = products.merge(items, how="inner", on="product_id").merge(orders, how = "inner", on="order_id")
    ...
    aggregations = delivered.groupby('order_id')[['freight_value', 'product_weight_g']].sum().reset_index(drop = False)
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    holidays = pd.read_sql("SELECT * FROM public_holidays", database)
   ...
   # Converting the order_purchase_timestamp column to datetime.
    ...
    # and assigning the result to the order_purchase_ammount_per_date variable.
    order_purchase_ammount_per_date = (
        filtered_dates
        .resample("D", on="order_purchase_timestamp")['order_id']
        .count()
        .reset_index()
        )
    # Creating the 'holidays' column based on whether a certain date is holiday or not.
